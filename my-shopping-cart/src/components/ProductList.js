...
const ProductList = ({ updateCartData }) => {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        const response = await axios.get('http://localhost:8000/public/api/get_product.php');
        const productList = response.data;

        // Gọi API để lấy danh sách sản phẩm trong giỏ hàng
        const cartResponse = await axios.get('http://localhost:8000/public/api/view_cart.php');
